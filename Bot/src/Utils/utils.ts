...
export default class Utils {
    ...
    public SevenTVEmoteURLRegex: RegExp =
        /(https:\/\/(www)?\.?7tv\.app\/emotes\/)?([0-9abcdef]{24})/gim;
    ...
    isSafeMessage(message: string): Boolean {
        let sanitizedMessage = this.removeAccents(
            this.removeBypassCharacters(message)
        );
        if (
            sanitizedMessage.match(/(im|i\sam|i'm)\s(1|2|3|4|5|6|7|8|9)(1|2)/gi)
        ) {
            return false;
        }
        ...
        if (
            sanitizedMessage.match(
                /((n|ñ|Ñ|ń|ņ|ň|ɲ|ŋ|ƞ|ǹ|ȵ|ɳ|ṉ|ṋ|ṅ|ṇ|\/\\\/|\|\\\|)[_\.\-\s]?[!1i|l][_\.\-\s]?[GgbB6934Q🅱qğĜƃ၅5]{2,3})(a|e|4)/gi
            )
        ) {
            return false;
        }
    ...
    formatMessage(message: string, noQuotations: boolean = false): string {
        // pajaDank
        return this.messageContainsAscii(
            this.isSafeMessage
                ? `${noQuotations ? "" : '"'}` +
                      message +
                      `${noQuotations ? "" : '"'}`
    ...
    canUseCommand(
        ...
        if (
            ob.blockedUsers.filter(
                (blockedUser) => blockedUser.userId === msg.userInfo.userId
            ).length > 0
        ) {
            if (
                ob.blockedUsers
                    .filter(
                        (blockedUser) =>
                            blockedUser.userId === msg.userInfo.userId
                    )[0]
        ...
        if (!ob.debug) {
            if (ob.cooldowns.userCooldowns.has(`${user}-${command.name}`)) {
                ...
                ob.cooldowns.userCooldowns.set(
                    `${user}-${command.name}`,
                    Date.now() + command.userCooldown * 1000
                );
                setTimeout(() => {
                    ob.cooldowns.userCooldowns.delete(
                        `${user}-${command.name}`
                    );
            ...
            if (
                ob.cooldowns.channelCooldowns.has(
                    `${channel.id}-${command.name}`
                )
            ) {
                return false;
            } else {
                ob.cooldowns.channelCooldowns.set(
                    `${channel.id}-${command.name}`,
                    Date.now() + command.channelCooldown * 1000
                );
                setTimeout(() => {
                    ob.cooldowns.channelCooldowns.delete(
                        `${channel.id}-${command.name}`
                    );
            ...
            ob.logger.info(
                `Ignored cooldown for ${user}-${command.name}`,
                "ob.utils"
            );
        }
        if (command.permissions) {
            if (
                !hasPermission(command.permissions, user, channel.channel, msg)
            ) {
                return false;
            }
        }
    ...
    humanizeTime(
        ms: number,
        depth: "seconds" | "minutes" | "hours" | "days" | "auto" = "auto"
    ): string {
    ...
    async unshortenURL(url: string): Promise<string> {
        let unshortendData = await ob.api.get<UnshortenMeResponse>(
            `https://unshorten.me/json/${encodeURIComponent(url)}`,
            86400 * 7
        );
        if (unshortendData.error) return url;

        return unshortendData.data.response.data
            ? unshortendData.data.response.data.resolved_url
            : url;
    }
    ...
    async resolveUserByUsername(
        ...
        let user = await ob.api.get<IvrFiUser>(
            `https://api.ivr.fi/twitch/resolve/${login}`,
            ttl
        );
    ...
    async resolveUserById(id: string, ttl: number = 300): Promise<IvrFiUser> {
        ...
        let user = await ob.api.get<IvrFiUser>(
            `https://api.ivr.fi/twitch/resolve/${id}?id=true`,
            ttl
        );
        if (user.error) return null;
        return user.data.response.data;
    ...
    async getSubage(login: string, channel: string): Promise<IvrFiSubage> {
        ...
        let resp = await ob.api.get<IvrFiSubage>(
            `https://api.ivr.fi/twitch/subage/${login}/${channel}`,
            300
        );
    ...
    async upload(text: string): Promise<string> {
        ...
        form.append(
            "attachment",
            fs.createReadStream(`./temp_${uniqueStr}.txt`)
        );
    ...
    async isIvrFiLoggedChannel(channel: string): Promise<boolean> {
        let resp = await ob.api.get<LogsIvrFiChannels>(
        ...
        if (
            resp.data.response.data.channels
                .map((channel: { userID: string; name: any }) => channel.name)
                .includes(channel)
        )
            return true;
        else return false;
    ...
    async smartObfuscate(
        _channel: Channel | SimplifiedChannel | string,
        user: string,
        author: string
    ): Promise<string> {
        return user;
    ...
    async get7tvChannelEmotes(channel: string): Promise<Emote[]> {
        ...
        if (resp.error) {
            ...
            ob.logger.warn(
                `Error getting 7tv emotes for ${channel}: ${resp.error}`,
                "ob.utils"
            );
            return [];
        }
    ...
    async invalidate7tvChannelEmotesCache(channel: string): Promise<void> {
        await ob.redis.del(
            ob.config.redisPrefix +
                ":" +
                "cache:api:" +
                ob.utils.generateMD5(
                    `https://api.7tv.app/v2/users/${channel}/emotes`
                )
        );
    }
    ...
    async get7tvGlobalmotes(): Promise<Emote[]> {
        ...
        if (resp.error) {
            ob.logger.warn(
                `Error getting 7tv global emotes: ${resp.error}`,
                "ob.utils"
            );
            return [];
        }
    ...
    async get7tvUserData(user: string): Promise<SevenTVRESTUserResponse> {
        ...
        if (resp.error) {
            ob.logger.warn(
                `Error getting 7tv user data for ${user}: ${resp.error}`,
                "ob.utils"
            );
            return null;
        }
    ...
    async getBttvChannelEmotes(channelId: string): Promise<Emote[]> {
        ...
        if (resp.error) {
            if (resp.error.code === "404") return [];
            ob.logger.warn(
                `Error getting bttv emotes for ${channelId}: ${resp.error}`,
                "ob.utils"
            );
            return [];
        }
    ...
    async getBttvGlobalEmotes(): Promise<Emote[]> {
        ...
        if (resp.error) {
            ob.logger.warn(
                `Error getting bttv global emotes: ${resp.error}`,
                "ob.utils"
            );
            return [];
        }

        return resp.data.response.data.map(
