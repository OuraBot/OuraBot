...
class suggestCommand extends Command {
    name = 'masscrossbanslow';
    description = 'Crossban multiple users across channels defined in config with a 1.2 second delay between each ban.';
    ...
    execute = async (user: string, channel: string, args: string[]): Promise<CommandReturnClass> => {
        ...
        chatClient.say(
            channel,
            `@${user}, crossbanning ${users.length} users across ${config.crossbanchannels.length} channels. This will take about ${Math.round(
                users.length * config.crossbanchannels.length * 1.2
            )} seconds`
        );
