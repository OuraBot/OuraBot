                    ...
                    if (term.ignorepermissions) {
                        if (term.channel === channel.replace('#', '')) {
                            ...
                            if (regex.test(message)) {
                                if (term.response.includes('{newline}')) {
                                    if ((await chatClient.getMods(channel)).toLowerCase().includes(process.env.CLIENT_USERNAME) || (await chatClient.getVips(channel)).toLowerCase().includes(process.env.CLIENT_USERNAME)) {
                        ...
                        if (msg.userInfo.isMod || msg.userInfo.isBroadcaster) {
                            ...
                            if (term.channel === channel.replace('#', '')) {
                                ...
                                if (regex.test(message)) {
                                    if (term.response.includes('{newline}')) {
                                        if ((await chatClient.getMods(channel)).toLowerCase().includes(process.env.CLIENT_USERNAME)) {
                                            let msgs = term.response.split('{newline}');
                                            for (let msg of msgs) {
                                                chatClient.say(channel, sanitizeMessage(msg.replace(/{user}/g, user)));
                                            }
                                ...
                                if (!onCooldown) {
                                    ...
                                    if (customCommand.response.match(/(GET|INCR){[A-z]{4,10}}/g)) {
                                        ...
                                        if (customCommand.response.match(/^REPEAT\([0-9]{1,3}\)\s/)) {
                                            ...
                                            let response = customCommand.response.replace(/^REPEAT\([0-9]{1,3}\)\s/, '');
                                            if (
                                                (await chatClient.getMods(channel)).toLowerCase().includes(process.env.CLIENT_USERNAME) ||
                                                (await chatClient.getVips(channel)).toLowerCase().includes(process.env.CLIENT_USERNAME)
                                            ) {
                                                for (let i = 0; i < repeatCount && i < 150; i++) {
                                                    await new Promise((resolve) => setTimeout(resolve, 500));
                                                    chatClient.say(channel, sanitizeMessage(response));
            ...
            if (_cmds.get(targetCmd)) {
                ...
                async function executeCommand() {
                    if (await handleCooldown()) {
                        ...
                        if (command.requiresMod && !(await chatClient.getMods(channel)).toLowerCase().includes(process.env.CLIENT_USERNAME)) {
